...
cd mapperproxy
for %%x in (pypy.exe) do if not [%%~$PATH:x]==[] echo "Running mapper with PyPy." && pypy.exe -B startmapper.py raw && goto :finished
for %%x in (python.exe) do if not [%%~$PATH:x]==[] echo "Running mapper with Python." && python.exe -B startmapper.py raw && goto :finished
